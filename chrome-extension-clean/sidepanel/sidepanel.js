...
class PromptLibrarySidePanel {
  ...
  handleDragStart(e) {
    ...
    // Store the dragged folder id and original position
    e.dataTransfer.setData('text/plain', folderId);
    e.dataTransfer.effectAllowed = 'move';

    this.draggedFolderId = folderId;
    this.draggedElement = folderItem;
    this.originalPosition = this.getFolderPosition(folderId);
    this.currentPreviewPosition = null;

    // Store original folder data for real-time preview
    const folders = this.libraryData.folders || [];
    this.originalFolderData = JSON.parse(JSON.stringify(folders));

    // Add visual feedback
    folderItem.classList.add('dragging');

    // Create a placeholder where the dragged item was
  ...
  applyFinalPosition() {
    ...
    if (draggedFolder) {
      ...
      const oldSiblings = previewData.filter(f => f.parentId === draggedFolder.parentId && f.id !== this.draggedFolderId);
      oldSiblings.forEach((folder, index) => {
        folder.sortOrder = index;
      });

      // Update dragged folder
      draggedFolder.parentId = this.currentPreviewPosition.parentId;
      draggedFolder.sortOrder = this.currentPreviewPosition.index;

      // Update new siblings
      const newSiblings = previewData.filter(f => f.parentId === this.currentPreviewPosition.parentId && f.id !== this.draggedFolderId);
      newSiblings.forEach(folder => {
        if (folder.sortOrder >= this.currentPreviewPosition.index) {
          folder.sortOrder++;
        }
      });

      // Temporarily update library data for rendering
      const originalFolders = this.libraryData.folders;
      this.libraryData.folders = previewData;

      // Re-render with preview data
      this.renderLibrary();

      // Restore the dragging state that gets lost in re-render
      setTimeout(() => {
        const newDraggedElement = document.querySelector(`[data-folder-id="${this.draggedFolderId}"]`);
        if (newDraggedElement) {
          newDraggedElement.classList.add('dragging');
